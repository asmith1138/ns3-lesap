...
RoutingExperiment::SetProtocol(std::string protocol)
{
    ...
    m_CSVfileName = m_protocolName + ".csv";
}
...
main(int argc, char* argv[])
{
    ...
    RoutingExperiment experimentDSR;
    experimentDSR.SetProtocol("DSR");
    experimentDSR.TurnOffFlowmon();
    experimentDSR.CommandSetup(argc, argv);
    experimentDSR.Run();

    std::cout << "**OLSR**" << std::endl;
    RoutingExperiment experimentOLSR;
    experimentOLSR.SetProtocol("OLSR");
    experimentOLSR.CommandSetup(argc, argv);
    experimentOLSR.Run();

    std::cout << "**DSDV**" << std::endl;
    RoutingExperiment experimentDSDV;
    experimentDSDV.SetProtocol("DSDV");
    experimentDSDV.CommandSetup(argc, argv);
    experimentDSDV.Run();
    return 0;
}
